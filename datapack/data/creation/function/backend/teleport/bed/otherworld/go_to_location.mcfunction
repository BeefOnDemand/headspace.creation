...
$execute at @s as @a[distance=..1.5,sort=furthest,predicate=creation:on_otherworld_bed,predicate=!creation:sneaking,predicate=creation:creation_user] in $(dimension) if score @s cr.check.BedLocated matches 1 run tp $(x) $(y) $(z)
