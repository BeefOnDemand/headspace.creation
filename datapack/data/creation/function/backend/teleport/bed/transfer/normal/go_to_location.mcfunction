$execute in $(dimension) positioned $(x) $(y) $(z) unless entity @s[tag=cr.tag.ForceloadedTransferNormalChunk] store success score @s cr.check.PermanentChunk run forceload query ~ ~
...
$execute in $(dimension) if score @s cr.check.PermanentChunk matches 0 run tag @s add cr.tag.ForceloadedTransferNormalChunk
...
$execute if score @s cr.check.BedLocated matches 1 at @s as @a[distance=..2,sort=furthest,predicate=creation:on_normal_transfer_bed,predicate=!creation:sneaking,predicate=creation:creation_user] in $(dimension) run tp $(x) $(y) $(z)
