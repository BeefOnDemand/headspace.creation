...
$execute if score @s cr.check.BedLocated matches 1 at @s as @a[distance=..2,sort=furthest,predicate=creation:on_superflat_transfer_bed,predicate=!creation:sneaking,predicate=creation:creation_user] in $(dimension) run tp $(x) $(y) $(z)
